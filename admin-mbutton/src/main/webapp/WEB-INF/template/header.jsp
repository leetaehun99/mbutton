...
loginDiv = function(){
	...
	$.blockUI({message:$("#loginContener"),css:{border: 'none',width: '455px', left:'35%',top:'100px',backgroundColor:'none'}, overlayCSS : {opacity:0.5,border: 'none'}});
};

createMemberDiv = function(){
	$.blockUI({message:$("#memberContener"),css:{border: 'none',width:'514px;',left:'35%',top:'100px',backgroundColor:'none'}, overlayCSS : {opacity:0.5,border: 'none'}});
};
...
var userInfoAjax = {
	success : function(resultData) {
		if( resultData != null ) {
			...
			$.blockUI({message:$("#updateMemberContener"),css:{border: 'none',width:'514px;',left:'35%',top:'100px',backgroundColor:'none'}, overlayCSS : {opacity:0.5,border: 'none'}});
		} else {
...
<div class="headerTop">
	...
	<div class="headerTopInner" <c:if test="${USER.userLevCd == '00001'}">style="height:133px;"</c:if> <c:if test="${USER.userLevCd == '00002'}">style="height:100px;"</c:if> >
		<ul>
			<li class="userNm">
			...
			<c:if test="${hMenuList != null}">
				<c:forEach var="menu" items="${hMenuList}" varStatus="idx">
					<c:if test="${menu.menuLev == '1'}">
						<c:if test="${USER.userLevCd != '00003' && (siteType == menu.siteType || menu.siteType == 'ALL')}">
...
<div id="loginContener" style="display:none;">
	<form id="loginFrm" name="loginFrm" method="post">
	<div class="mainTable" style="cursor:default;">
