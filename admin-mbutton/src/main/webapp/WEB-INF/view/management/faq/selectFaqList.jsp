...
<script type="text/javascript">
	$(function() {
		...
		$("#createFaq").click(function() { 
			$("#faqForm").attr("action", "/faq/createFaqForm.doo");
			$("#faqForm").submit();
		});
	...
	setAction = function(){
		if($("#cd").val() == "01"){
			...
			$("#noticeForm").attr("action", "/faq/selectFaqList.doo");
		}else{
			$("#faqForm").attr("action", "/faq/selectFaqList.doo");
		}
	};
...
<form id="faqForm" name="faqForm" action="/faq/selectFaqList.doo"method="post">
	...
	<table class="table9">
		...
		<tfoot>
			<tr><td colspan="6"><span class="button medium icon"><span class="check"></span><a id="createFaq">등록</a></span></td></tr>
		</tfoot>
	</table>
	<!-- 페이지번호 -->
