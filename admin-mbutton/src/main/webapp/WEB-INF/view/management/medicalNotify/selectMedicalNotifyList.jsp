...
$(function() {
	$("#createMedicalNotify").click(function() { 
		$("#notifyForm").attr("action", "/medicalNotify/createMedicalNotifyForm.doo");
		$("#notifyForm").submit();
	});
	//검색
	...
	//삭제
	$("#deleteMedicalNotify").click(function() {
		...
		$("#notifyForm").attr("action", "/medicalNotify/selectMedicalNotifyCnt.json");
		$("#notifyForm").ajaxSubmit(selectMedicalNotifyCntAjax);
	});
...
readNotify = function(medicalNotify,medicalNotifySub){
	$("#notifyForm").attr("action", "/medicalNotify/selectMedicalNotify.doo");
	$("#medicalNotify","#notifyForm").val(medicalNotify);
	$("#medicalNotifySub","#notifyForm").val(medicalNotifySub);
	$("#notifyForm").submit();
}

readNotifyItem  = function(medicalNotify,medicalNotifySub){
...
searchVal = function(searchText){
	$("#searchType").val("MEDICAL_NOTIFY");
	$("#searchText").val(searchText);
	$("#searchList").click();
}

//하위 테이블 삭제존재유무 체크
var selectMedicalNotifyCntAjax = {
	success : function(resultData) {
		if( resultData.length > 0 ) {
			var msg = checkSelectMedicalNotifyCntAjax(resultData);
			alert(msg);
		} else {
			var checkConfirm = confirm("정말로 삭제하시겠습니까?");
			if(checkConfirm) {
				$("#notifyForm").attr("action", "/medicalNotify/deleteMedicalNotify.json");
...
function checkSelectMedicalNotifyCntAjax(resultData) {
	var str = "";
	str += "[분류번호, 분류서브번호] : ";
	for(var i =0; i<resultData.length; i++) {
		for(var j=0; j<delCheckArr.length; j++) {
			var a = resultData[i].medicalNotify + "_" + resultData[i].medicalNotifySub;
			if(a != delCheckArr[j]) {
				continue;
			}
			str += "[" + resultData[i].medicalNotify + ", " + resultData[i].medicalNotifySub + "]";
			str += " ";
		}
...
<form id="notifyForm" name="notifyForm" action="/medicalNotify/selectMedicalNotifyList.doo"method="post">
	...
	<div>
		...
		<tbody>
			<tr>
				<th>구분</th>
				<td>
					<ct:code name="searchType" type="select" groupCode="00078"  selectCode="${medicalNotifyVo.searchType}" defaultCode="MEDICAL_CD"/>
	...
	<table class="table5">
		<caption><!-- img src="/resources/img/Notify2.gif" --><span class="divAr"><ct:code name="rowPerPage" type="select" groupCode="00009"  selectCode="${medicalNotifyVo.rowPerPage}" eventNm="onchange" eventFn="javascript:commonRowPerChange(document.notifyForm,'/medicalNotify/selectMedicalNotifyList.doo');"/></span></caption>
		<colgroup>
			<col width="40px;">
			<col width="70px;">
			<col width="70px;">
		...
		<tbody id="notifyListTbody">
			<c:if test="${notifyList != null}">
				<c:forEach var="notify" items="${notifyList}">
					<tr class="trFocus">
						<td>
							<input type="checkBox"  name="arrDel" class="arrDel" value="${notify.medicalNotify }_${notify.medicalNotifySub}" />
						...
						<td><span class="button medium icon"><span class="check"></span><a onclick="javascript:readNotifyItem('${notify.medicalNotify}','${notify.medicalNotifySub}');">수가</a></span></td>
						<td><span class="button medium icon"><span class="check"></span><a onclick="javascript:readNotify('${notify.medicalNotify}','${notify.medicalNotifySub}');">상세</a></span></td>
						
						<td>${notify.medicalNotify}</td>
						<td>${notify.medicalNotifySub}</td>
		...
		<tfoot>
			<tr>
				<td colspan="7">
					...
					<span id="delete"><span class="button medium icon"><span class="check"></span><a id="deleteMedicalNotify">삭제</a></span></span>
				</td>
			</tr>
		</tfoot>
	</table>
