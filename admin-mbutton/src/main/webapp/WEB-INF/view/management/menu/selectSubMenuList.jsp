...
<script type="text/javascript">
	$(function() {
		$("#menuParentForm").attr("action", "/menu/selectParentMenu.json");
		$("#menuParentForm").ajaxSubmit(selectParentMenuAjax);
		...
		//그룹코드 목록
		$("#selectSubMenuList").click(function(){
...
<form id="menuParentForm" name="menuParentForm" action="/menu/selectMenuList.doo"method="post">
	<input type="hidden"  name="menuId" value=""/>
	<input type="hidden"  name="menuYn" value="N"/>
	<input type="hidden"  name="grpId" value="${menuVo.grpId}"/>
	<input type="hidden"  name="currentPage" value="1"/>
</form>

<form id="menuForm" name="menuForm" action="/menu/selectMenuList.doo"method="post">
	<input type="hidden" id="menuId" name="menuId" value=""/>
	<input type="hidden" id="menuYn" name="menuYn" value="N"/>
	<input type="hidden" id="grpId" name="grpId" value="${menuVo.grpId}"/>
	...
	<table class="table5">
		<caption><img src="/resources/img/menuList.gif"><span class="divAr"><ct:code name="rowPerPage" type="select" groupCode="00009"  selectCode="${menuVo.rowPerPage}" eventNm="onchange" eventFn="javascript:commonRowPerChange(document.menuForm,'/menu/selectSubMenuList.doo');"/></span></caption>
		<colgroup>
			<col width="7%">
			<col width="11%">
			<col width="*">
		...
		<tbody id="menuListTbody">
			<c:if test="${menuList != null}">
				<c:forEach var="menu" items="${menuList}">
					<tr class="trFocus" onclick="javascript:readMenu('${menu.menuId}');">
						<td>${menu.menuId}</td>
	...
	<table class="table5">
		...
		<tbody>
			<tr>
				...
				<td><input type="text" id="linkUrl" 	name="linkUrl" 		value="" size="32" maxlength="40"/></td>
				...
				<td><select id="grpSelectId" name="grpSelectId" ></select></td>
				<td><input type="text" id="sortSeq" 	name="sortSeq" 		value="" size="9" maxlength="2"/></td>
		...
		<tfoot>
			<tr>
				<td colspan="8">
					...
					<span class="button medium icon"><span class="check"></span><a id="selectSubMenuList">목록</a></span>
				</td>
			</tr>
		</tfoot>
	</table>
