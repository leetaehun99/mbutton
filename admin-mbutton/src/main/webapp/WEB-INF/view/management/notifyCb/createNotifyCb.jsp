	...
	$(function() {
		$("#createDrugNotify").click(function(){ //고시 등록 
			if(valueCheck()) {
				...
				$("#notifyForm").attr("action", "/notifyCb/createNotifyCb.json");
				$("#notifyForm").ajaxSubmit(createDrugCbNotifyAjax);	
			}
		});

		$("#selectDrugNotify").click(function(){ //고시 목록 이동
			$("#notifyForm").attr("action", "/notifyCb/selectNotifyCbList.doo");
			$("#notifyForm").submit();
		});
...
<form id="notifyForm" name="notifyForm" action="/notifyCb/selectNotifyCbList.doo" method="post" >	
	<!--검색 조건  -->
