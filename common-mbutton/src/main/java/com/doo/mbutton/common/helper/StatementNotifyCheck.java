...
public class StatementNotifyCheck {
	 ...
	 * 진료자 나이
	 */
	private int getAge(String birthy,String dt){
		int aged=0;
		try{
			//진료일
			...
			//환자나이
			int dy = Integer.parseInt(birthy.substring(0, 4));
			int dm = Integer.parseInt(birthy.substring(4, 6));
			int dd = Integer.parseInt(birthy.substring(6, 8));
	...
	public void notifyCheck(StatementVo statementVo, ErrorCheckVo errorCheckVo){
		...
		String registerId = statementVo.getRegisterId();
		String ymd = statementVo.getTrtStartDt();//생년월일
		String birthy = ""; 
		if(errorCheckVo.getBirthy()!=null && errorCheckVo.getBirthy()!=""){
			birthy=errorCheckVo.getBirthy();
		}else{
			birthy = statementVo.getMedicalStatementVo().getBirthDy();
		}
		int age = getAge(birthy,ymd );//나이
		int ptSize = 0; //약품 갯수
		statementVo.setAge(age);
		//입원 여부 Check
		...
		List<String> allMedicalInsurCd= new ArrayList<String>(); //수가 코드 List
		...
		//원내 처방 값 세팅
		setTrtCdList(treatmentStatementList,specificDetailList,drugCdList2,drugAllCdList,mainDrugCdList2,mainDrugAllCdList,medicalCdList,trtCdList,castCdList,oneDayCdList,kkCdList,checkHospital,mapList,allMedicalInsurCd);
		...
		if(mainDrugAllCdList.size()>0){
			//인증상병(약가) 체크
			if(mainDrugAllCdList.size()>0) diseaCheck(errorVo,diseaCdList,mainDrugAllCdList,errorCheckList,mapList,registerId);
			//인증상병(수가) 체크
			//if(allMedicalInsurCd.size()>0) medicalInsurCheck(errorVo,diseaCdList,allMedicalInsurCd,errorCheckList,mapList,registerId);
			//연령별Check 저함량 JX999
			...
			//병용금기Check
			if(mainDrugAllCdList.size()>0) extCheck2(errorVo,mainDrugCdList1,mainDrugCdList2,errorCheckList,mapList);
			//단일금기Check
			if(mainDrugAllCdList.size()>0) extCheck6(errorVo,mainDrugCdList1,mainDrugCdList2,errorCheckList,mapList);
		}
	 ...
	 * 원외 처방 값 세팅
	 */
	private void setPrtCdList(List<PrescriptnStatementVo> prescriptnStatementList,List<SpecificDetailVo> specificDetailList,List<String> drugCdList,List<String> drugAllCdList,List<String> mainDrugCdList1,List<String> mainDrugCdList,List<Map<String,Object>> mapList){
		if(prescriptnStatementList!=null){
			for(int x=0; x<prescriptnStatementList.size(); x++){
				...
				map.put("CNT",  (Float.parseFloat(prtVo.getDorseTotCnt()))+"");
				for(int y=0; y<specificDetailList.size(); y++){
					if("3".equals(specificDetailList.get(y).getPrscrptnDivd())){
						if(prtVo.getLnNum().equals(specificDetailList.get(y).getLnNum())){
							speList.add(specificDetailList.get(y).getSpeDetailDivd());
						}
					}
				}
	...
	private void setTrtCdList(List<TreatmentStatementVo> treatmentStatementList,List<SpecificDetailVo> specificDetailList,List<String> drugCdList,List<String> drugAllCdList,List<String> mainDrugCdList2,List<String> mainDrugCdList,List<String> medicalCdList,List<String> trtCdList,List<String> castCdList,List<String> oneDayCdList,List<String> kkCdList,boolean checkHospital,List<Map<String,Object>> mapList, List<String> allMedicalInsurCd){
		...
		if(treatmentStatementList!=null){
			...
			for(int x=0; x<treatmentStatementList.size(); x++){
				...
				map.put("CNT",  (Float.parseFloat(trtVo.getDorseTotCnt()))+"");
				for(int y=0; y<specificDetailList.size(); y++){
				...
				if("1".equals(trtVo.getCdDivd())){ // 수가
					allMedicalInsurCd.add(trtVo.getCd());
	 ...
	 * 인증상병(약가) Check
	 */
	private void diseaCheck(ErrorCheckVo errorVo,List<String> diseaCdList,List<String> mainDrugAllCdList,List<ErrorCheckVo> errorCheckList,List<Map<String,Object>> mapList,String registerId){
		errorVo = new ErrorCheckVo();
		errorVo.setMainDrugAllCdList(mainDrugAllCdList);
		errorVo.setDiseaCdList(diseaCdList);
		errorVo.setRegisterId(registerId);
	 ...
	 * 인증상병(수가) Check
	 */
	private void medicalInsurCheck(ErrorCheckVo errorVo,List<String> diseaCdList,List<String> allMedicalInsurCd, List<ErrorCheckVo> errorCheckList,List<Map<String,Object>> mapList,String registerId){
		errorVo = new ErrorCheckVo();
		errorVo.setAllMedicalInsurCd(allMedicalInsurCd);
		errorVo.setDiseaCdList(diseaCdList);
		errorVo.setRegisterId(registerId);
		List<ErrorCheckVo> list = new ArrayList<ErrorCheckVo>();
		if (allMedicalInsurCd != null && allMedicalInsurCd.size() > 0)
			list = statementMapper.selectMedicalCheck(errorVo);
			errorCheckList.addAll(errorCheckList.size(),list);
	}
	 ...
	 * 연령별 저함량 배수Check
	 */
	private void extCheck1(ErrorCheckVo errorVo,List<String> mainDrugCdList1,List<String> mainDrugCdList2,List<String> mainDrugAllCdList,List<String> drugCdList,List<ErrorCheckVo> errorCheckList,int age,List<Map<String,Object>> mapList){
		errorVo = new ErrorCheckVo();
				...
				if("0001".equals(extVo.getNotify()) && "C".equals(extVo.getExtDiv1())){	//A:주 B:개월 C:세, F:경구제,G:주사제
					...
					for(int x=0; x<mapList.size(); x++){
						if("3".equals(mapList.get(x).get("TYPE")) &&  extVo.getMainDrugCd().equals(mapList.get(x).get("MAIN_DRUG_CD"))){
							...
							List<String> subList = (List<String>)mapList.get(x).get("SPELIST");
							
							for(int v = 0; v<subList.size(); v++){
								String speCd = (String)subList.get(v);
								
								if(extVo.getSpecialCd().equals(speCd)){
									checkFlag = true; break;
								}
							}
							if(!checkFlag)extErrorList.add(extVo);
						}
					}
					...
					for(int x=0; x<mapList.size(); x++){
						if("3".equals(mapList.get(x).get("TYPE")) && extVo.getMainDrugCd().equals(mapList.get(x).get("MAIN_DRUG_CD"))){
							//System.out.println(Float.parseFloat(mapList.get(x).get("CNT")) + "\t" + Float.parseFloat(extVo.getVal()));
							if(Float.parseFloat(mapList.get(x).get("CNT").toString()) > Float.parseFloat(extVo.getVal())){
								extVo.setErrMsg(extVo.getErrMsg().replace("$[param1]", (Float.parseFloat(extVo.getVal())/7)+""));
								extErrorList.add(extVo);
							}
						}
					}
				}else if("0017".equals(extVo.getNotify())){	//골대사성 의약품 검사결과
	 ...
	 * 병용금기 Check
	 */
	private void extCheck2(ErrorCheckVo errorVo,List<String> mainDrugCdList1,List<String> mainDrugCdList2,List<ErrorCheckVo> errorCheckList,List<Map<String,Object>> mapList){
		errorVo = new ErrorCheckVo();
		if(mainDrugCdList1.size()>0){
			errorVo.setMainDrugAllCdList(mainDrugCdList1);
			List<ErrorCheckVo> list = statementMapper.selectCbCheck(errorVo);
			list = findSetDrugCd(list,mapList,"2");
			errorCheckList.addAll(errorCheckList.size(),list);
		}
		
		if(mainDrugCdList2.size()>0){
			errorVo.setMainDrugAllCdList(mainDrugCdList2);
			List<ErrorCheckVo> list = statementMapper.selectCbCheck(errorVo);
			list = findSetDrugCd(list,mapList,"2");
			errorCheckList.addAll(errorCheckList.size(),list);
		}
	 ...
	 * 단일금기 Check
	 */
	private void extCheck6(ErrorCheckVo errorVo,List<String> mainDrugCdList1,List<String> mainDrugCdList2,List<ErrorCheckVo> errorCheckList,List<Map<String,Object>> mapList){
		errorVo = new ErrorCheckVo();
	...
	private List<ErrorCheckVo> countCheck(ErrorCheckVo errorCheckVo){
		...
		logger.info(temp.getNotify()+"-"+temp.getNotifySub()+"\t"+temp.getCd()+"\t"+"갯수:"+temp.getVal()+"\t합:"+sum);
		logger.info(title+"\t"+"갯수:"+temp.getVal()+"\t합:"+sum);
	...
	private List<ErrorCheckVo> validationCheck(ErrorCheckVo errorCheckVo){
		...
		logger.info("CNT = "+cnt +"\t"+"VAL = "+errorList.size());
	...
	private List<ErrorCheckVo> findSetDrugCd(List<ErrorCheckVo> list,List<Map<String,Object>> mapList,String type){
		
		//System.out.println("list .size = "+list.size());
		for(int x=0; x<list.size(); x++){
			for(int y=0; y<mapList.size(); y++){
				Map<String,Object> map = mapList.get(y);
				if("3".equals(map.get("TYPE"))){
					try{
						if((map.get("MAIN_DRUG_CD").toString().trim()).equals(list.get(x).getMainDrugCd().trim())){
							list.get(x).setCd((String)map.get("DRUG_CD"));
							if(!type.equals("0")){
								String tempCds[] = list.get(x).getVal().split(",");
								if(type.equals("2")){ 
									list.get(x).setErrMsg(list.get(x).getErrMsg().replace("$[param1]", map.get("DRUG_CD").toString())); 
	...
	private String findDrugCdLists(String[] array ,List<Map<String,Object>> mapList){
		String result="";
		try{
			if(array.length>0){
					for(int y=0; y<mapList.size(); y++){
