...
<script type="text/javascript">
	...
	$(function() {
		...
		$("#deleteStatement").click(function(){
			...
			if(checkFlag){
				checkFlag = confirm("정말로 삭제하시겠습니까?");
				if(checkFlag){
					$.blockUI({message:$("#loadingDiv"),css:{border:'none',backgroundColor:'',color:'',padding:'20px'},overlayCSS : {opacity:0.0}});
			...
			if (percentComplete == 100) {
				...
				$.ajax({
						...
						success : function(result){
							...
							if(parseInt(json.ccurrentVal)>1){
								$("#progressbar2").width('100%');
								$("#percent2").html('100%');
								$("#progressbar3").width(parseInt( (parseInt(json.ccurrentVal) / parseInt(json.ctotalVal)* 100), 10)+'%');
								$("#percent3").html(parseInt( (parseInt(json.ccurrentVal) / parseInt(json.ctotalVal)* 100), 10)+'%');
							}else{
								$("#percent3").html("유효성 체크중입니다..");
							}
						}
					});
				},1000);
	...
	readStatement = function(recpCstClmSeq,clmNum,hspId){
		...
		$("#clmNum").val(clmNum);
		$("#hspId").val(hspId);
		if(checkBrowser()=="IE")	pop('','frontStatement','1010', '730');
		else if(checkBrowser()=="FF")	pop('','frontStatement','1010', '730');
		else pop('','frontStatement','1110', '800');
		$("#statementForm").attr("action", "/statement/selectStatement.doo");
		$("#statementForm").attr("target", "frontStatement");
		$("#statementForm").submit();
...
<form id="statementForm" name="statementForm" action="" method="post" enctype="multipart/form-data">
				...
				<tr>
					...
					<td style="padding:2px;">
						...
						<div style="padding-top:10px;">
							<div style="width:80px; float:left;text-align:right;padding: 7px;">초기화 : </div> 
							<div id="progressbox" style=" float:left;">
								<div id="progressbar"></div>
								<div id="percent"></div>
							</div><div class="clear"></div>
							
							<div style="width:80px; float:left;text-align:right;padding: 7px;">유효성체크 : </div> 
							...
							<div style="width:80px; float:left;text-align:right;padding: 7px;">사전점검 : </div> 
							<div id="progressbox3" style=" float:left;">
	...
	<table class="table9">
		...
		<tfoot><tr><td colspan="8" align="right"><span class="button medium icon"><span class="check"></span><a id="deleteStatement">삭제</a></span> </td></tr></tfoot>
	</table>
	<!-- 페이지번호 -->
